...
Зафиксировано по итогам grill-me интервью. Ничего из этого ещё не установлено и не реализовано — это согласованный план, а не код.
...
**MVP-функционал:** поиск в TMDB → добавление в список (без ручного ввода) → отметка «просмотрено» (bool-поле) → удаление. Дубликаты блокирует уникальный индекс `(user_id, tmdb_id, media_type)`. Тестов в MVP нет — один Playwright e2e добавим позже, когда основной сценарий стабилизируется.
